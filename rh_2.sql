...
prtcpnt_e.refrl_id, prtcpnt_e.prtcpnt_nm, prtcpnt_e.pat_id as sg_uid, prtcpnt_e.scrning_id as scrning_id, prtcpnt_e.stdy_id as protocol,
...
prtcpnt_e.prtcpnt_enrl_dt as enrolled_date, prtcpnt_e.hcp_prtcpnt_rndmized_dt as rh_randomized_date, as_date(tmdh.active_randomized) as active_randomized, high_risk_ind,
...
--Days Unprocessed
case 
    when prtcpnt_e.prtcpnt_stat_cd = 'Received' then datediff(day, prtcpnt_e.ref_dt, current_date)
    --when prtcpnt_e.prtcpnt_stat_cd = 'Contact Attempted' then datediff(day, prtcpnt_e.prtcpnt_stat_last_updt_dt, current_date)
    --when prtcpnt_e.prtcpnt_stat_cd = 'Unable to Reach' then datediff(day, prtcpnt_e.prtcpnt_stat_last_updt_dt, current_date)
    else null end as days_unprocessed,
--Contact Attempted
case 
...
age,
stdy_site_name
,AVG(days_unprocessed) OVER (PARTITION BY stdy_site_name) AS average_days_unprocessed
,site_number
,site_country
,site_country_code
,site_city,state
...
group by protocol ,ref_date,week_ends_sun, utm_source,utm_medium,utm_z,high_risk_ind,clncl_stdy_site_init_vst_sched_dt,initial_visit_occurred_date,informed_consent_date,gender_cd,randomized_date,race,ethnicity,age,stdy_site_name,site_number,site_country,site_country_code,site_city,state,site_zip,Category,channel,vendor
)
